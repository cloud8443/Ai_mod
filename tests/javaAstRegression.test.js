...
test('java regression fixture: forge->fabric 1.20.6 applies import/class/prefix mappings', () => {
  ...
  assert.ok(plan.results[0].appliedRuleIds.some((id) => id.startsWith('ast:')));
});

test('java regression fixture: forge->fabric 1.21.0 applies ResourceLocation static factory', () => {
  const plan = runDeterministicRuleTransform({
    files: [{ path: 'ExampleMod.java', content: fixture('java-forge-input.java') }],
    sourceLoader: 'forge',
    targetLoader: 'fabric',
